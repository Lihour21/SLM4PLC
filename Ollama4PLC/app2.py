...
class Ollama4PLC:
    ...
    def generate_st_code(self, requirements: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        prompt = f"""You are an expert PLC programmer generating IEC 61131-3 Structured Text for the RuSTy compiler (https://plc-lang.github.io/rusty/).

TASK: Generate a COMPLETE, COMPILABLE Structured Text program from the following requirements.

CRITICAL SYNTAX RULES FOR RUSTY COMPILER:

1. PROGRAM structure (mandatory):
   PROGRAM ProgramName
   VAR_INPUT
   END_VAR
   VAR_OUTPUT
...
2. Variable declarations:
   - Format: VarName : DataType;
   - Each declaration ends with a semicolon (;)
   - Multiple vars same type: Var1, Var2 : INT;
   - Allowed types for this task: BOOL, INT only
   - ALL variables MUST be declared before use

3. Statements and semicolons:
   - EVERY executable statement ends with ;
   - Assignment: Var := Expression;
   - IF conditions: IF Expr THEN Stmt; Stmt; END_IF
   - ELSIF chains: ELSIF Expr THEN Stmt; Stmt;
   - ELSE block: ELSE Stmt; Stmt;
   - Block keywords (END_IF, END_VAR, END_PROGRAM, END_CASE, END_FOR, END_WHILE, END_REPEAT) NEVER take semicolons

4. Comments:
   - Use ONLY (* comment text *) format
...
7. FORBIDDEN for this task:
   - Function blocks: TON, TOF, CTU, CTD, TP, RTC, etc.
   - Arrays or complex data types
   - String operations
   - Real/floating point types (REAL, LREAL)
   - Direct addressing (%IX0.0, %QX0.0, etc.)

8. Variable naming:
   - Use descriptive PascalCase with underscores: Start_PB, Stop_PB, Motor_Run
   - All variables MUST be declared in VAR sections
   - No undeclared variables allowed

9. Program structure template:
   PROGRAM MainProgram
   VAR_INPUT
       (* all inputs - one per line, ends with ; *)
   END_VAR
   VAR_OUTPUT
       (* all outputs - one per line, ends with ; *)
   END_VAR
   VAR
       (* all internal variables - one per line, ends with ; *)
   END_VAR
   (* logic here - one statement per line, every statement ends with ; *)
   END_PROGRAM
...
2. Variables: List ALL variables with types and purposes
...
PROGRAM MainProgram
    VAR_INPUT
        (* declare each input on separate line with type, end with ; *)
    END_VAR
    VAR_OUTPUT
        (* declare each output on separate line with type, end with ; *)
    END_VAR
    VAR
        (* declare each internal on separate line with type, end with ; *)
    END_VAR

    (* Your logic here - one statement per line, every statement ends with ; *)
    (* Block terminators (END_IF, END_VAR, END_PROGRAM) NEVER have semicolons *)
